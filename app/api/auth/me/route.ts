...
export async function GET() {
    try {
        ...
        const canEdit = permMatrix.canEdit?.includes(role) || false;
        const canDelete = permMatrix.canDelete?.includes(role) || false;
        const canViewAudit = permMatrix.canViewAudit?.includes(role) || false;
        const canAccessSettings = permMatrix.canAccessSettings?.includes(role) || false;
        const canUpload = permMatrix.canUpload?.includes(role) || false;
