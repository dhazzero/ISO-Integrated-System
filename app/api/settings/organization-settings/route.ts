...
export async function GET() {
    try {
        ...
        return NextResponse.json({ message: 'Gagal mengambil pengaturan organisasi', error: (error as Error).message }, { status: 500 });
    }
}
...
export async function PUT(request: Request) {
    try {
        ...
        return NextResponse.json({ message: 'Pengaturan organisasi berhasil disimpan', settings: result });
    } catch (error) {
        console.error('Failed to update organization settings:', error);
        return NextResponse.json({ message: 'Gagal menyimpan pengaturan organisasi', error: (error as Error).message }, { status: 500 });
    }
}
