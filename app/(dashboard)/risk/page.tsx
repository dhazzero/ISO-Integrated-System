...
export default function RiskPage() {
    ...
    const sortRisks = (data: Risk[]) => {
        if (!sortConfig) return data;
        return [...data].sort((a, b) => {
            let aValue = (a as any)[sortConfig.key];
            let bValue = (b as any)[sortConfig.key];
            
            if (sortConfig.key === 'relatedStandards') {
                 aValue = a.relatedStandards ? a.relatedStandards.join(',') : '';
                 bValue = b.relatedStandards ? b.relatedStandards.join(',') : '';
            }
    ...
    return (
        <TooltipProvider> {/* <-- TAMBAHKAN WRAPPER INI */}
            <div className="container mx-auto px-4 py-6">
                    ...
                    {riskSummary.map((risk, index) => (
                        <Card key={index}>
                            <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
                                <CardTitle className="text-base font-bold">Risiko {risk.level}</CardTitle>
                                <AlertTriangle className={`h-12 w-12 ${risk.level === 'Tinggi' ? 'text-red-500' : risk.level === 'Sedang' ? 'text-amber-500' : 'text-green-500'}`} />
                            </CardHeader>
                            <CardContent>
                                <div className="text-3xl font-bold">{risk.count}</div>
                                <p className="text-xs text-muted-foreground">Total risiko dengan level {risk.level.toLowerCase()}</p>
                            </CardContent>
                        </Card>
                    ))}
                </div>
                ...
                <Card>
                    <CardHeader className="pb-2"><CardTitle className="text-base font-bold">Register Risiko</CardTitle><CardDescription>Daftar semua risiko yang teridentifikasi</CardDescription></CardHeader>
                    <CardContent>
                        <div className="overflow-x-auto">
                            <table className="w-full">
                                {/* --- PERBARUI HEADER TABEL --- */}
                                <thead><tr className="border-b">
                                    <th className="text-left py-3 px-4 cursor-pointer hover:bg-muted/50 select-none" onClick={() => handleSort('name')}>Nama Risiko{renderSortIcon('name')}</th>
                                    <th className="text-left py-3 px-4 cursor-pointer hover:bg-muted/50 select-none" onClick={() => handleSort('relatedStandards')}>Standar Terkait{renderSortIcon('relatedStandards')}</th>
                                    <th className="text-left py-3 px-4 cursor-pointer hover:bg-muted/50 select-none" onClick={() => handleSort('level')}>Level{renderSortIcon('level')}</th>
                                    <th className="text-left py-3 px-4 cursor-pointer hover:bg-muted/50 select-none" onClick={() => handleSort('likelihood')}>Kemungkinan{renderSortIcon('likelihood')}</th>
                                    <th className="text-left py-3 px-4 cursor-pointer hover:bg-muted/50 select-none" onClick={() => handleSort('impact')}>Dampak{renderSortIcon('impact')}</th>
