    ...
    const fetchData = async () => {
      ...
      try {
        ...
        setAuditReport({
          ...
          conformityStatement: auditData.conformityStatement || "",
          systemEffectiveness: auditData.systemEffectiveness || "",
          divergingOpinions: auditData.divergingOpinions || "",
        });
          ...
          <div className="lg:col-span-2 space-y-6">
            ...
            <Card>
              <CardHeader className="bg-blue-50/50 dark:bg-blue-950/20">
                <CardTitle className="font-bold text-blue-900 dark:text-blue-200">
                  Kesimpulan & Evaluasi Audit (ISO 19011:2018)
                </CardTitle>
              </CardHeader>
              <CardContent className="space-y-4 pt-4">
                {auditReport.conformityStatement && (
                  <div>
                    <label className="text-xs font-semibold uppercase tracking-wider text-muted-foreground">
                      Pernyataan Kesesuaian / Statement of Conformity
                    </label>
                    <p className="font-semibold text-blue-600 dark:text-blue-400 mt-1">
                      {auditReport.conformityStatement === 'Fully Conformant' && 'Memenuhi Seluruh Kriteria (Fully Conformant)'}
                      {auditReport.conformityStatement === 'Partially Conformant' && 'Memenuhi Sebagian Besar Kriteria dengan Temuan Minor (Partially Conformant)'}
                      {auditReport.conformityStatement === 'Non-Conformant' && 'Tidak Memenuhi Kriteria Utama / Temuan Mayor (Non-Conformant)'}
                      {auditReport.conformityStatement === 'Inconclusive' && 'Belum Dapat Dinilai Sepenuhnya (Inconclusive)'}
                      {!['Fully Conformant', 'Partially Conformant', 'Non-Conformant', 'Inconclusive'].includes(auditReport.conformityStatement) && auditReport.conformityStatement}
                    </p>
                  </div>
                )}
                {auditReport.systemEffectiveness && (
                  <div>
                    <label className="text-xs font-semibold uppercase tracking-wider text-muted-foreground">
                      Efektivitas Sistem Manajemen / Management System Effectiveness
                    </label>
                    <p className="text-sm mt-1 text-foreground leading-relaxed whitespace-pre-line">
                      {auditReport.systemEffectiveness}
                    </p>
                  </div>
                )}
                {auditReport.divergingOpinions && (
                  <div>
                    <label className="text-xs font-semibold uppercase tracking-wider text-muted-foreground">
                      Ketidaksepakatan yang Belum Terselesaikan / Unresolved Diverging Opinions (Clause 6.6.2-g)
                    </label>
                    <p className="text-sm mt-1 text-amber-700 dark:text-amber-400 leading-relaxed whitespace-pre-line bg-amber-50/50 dark:bg-amber-950/20 p-2.5 rounded-md border border-amber-200/50">
                      {auditReport.divergingOpinions}
                    </p>
                  </div>
                )}
                {!auditReport.conformityStatement && !auditReport.systemEffectiveness && !auditReport.divergingOpinions && (
                  <p className="text-sm text-muted-foreground italic">
                    Belum ada detail evaluasi ISO 19011 yang tercatat untuk audit ini.
                  </p>
                )}
              </CardContent>
            </Card>
