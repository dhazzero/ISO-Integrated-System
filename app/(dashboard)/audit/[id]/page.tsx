...
import { ArrowLeft, Edit, Calendar, User, Building, Shield, Target, CheckCircle2, AlertTriangle, Eye, FileText, Trash2 } from "lucide-react";
...
export default function AuditDetailPage() {
    ...
    const handleDeleteFinding = async (findingId: string, description: string) => {
        if (!confirm(`Apakah Anda yakin ingin menghapus temuan "${description.substring(0, 30)}..."?`)) return;
        
        try {
            console.log('[DELETE FINDING] Sending DELETE request for:', findingId);
            const res = await fetch(`/api/findings/${findingId}`, { 
                method: 'DELETE',
                headers: { 'Content-Type': 'application/json' },
            });
            
            console.log('[DELETE FINDING] Response status:', res.status, 'redirected:', res.redirected);
            ...
            if (!res.ok) {
                if (contentType.includes('application/json')) {
                    ...
                    throw new Error(errorData.message || `Gagal menghapus temuan (HTTP ${res.status})`);
                } else {
                    const textData = await res.text();
                    console.error('[DELETE FINDING] Non-JSON error response:', textData.substring(0, 500));
                    throw new Error(`Gagal menghapus temuan (HTTP ${res.status})`);
                }
            }
            
            const result = await res.json();
            console.log('[DELETE FINDING] Success:', result);
            
            toast({
                title: 'Berhasil',
                description: result.message || 'Temuan berhasil dihapus',
            });
            fetchAuditData();
        } catch (error) {
                ...
                <TabsContent value="detail">
                    <Card>
                        ...
                        <CardContent className="space-y-4 pt-6 grid grid-cols-1 md:grid-cols-3 gap-4">
                            ...
                            <div className="space-y-1"><p className="text-sm font-medium text-muted-foreground">Jenis</p><div><Badge variant={audit.auditType === 'Internal' ? 'default' : 'secondary'}>{audit.auditType}</Badge></div></div>
                                        ...
                                        <td className="p-4"><div className="flex space-x-1">
                                            ...
                                            {permissions?.canDelete && (
                                                <Button 
                                                    ...
                                                    size="icon" 
                                                    title="Hapus"
                                                    onClick={() => handleDeleteFinding(finding._id, finding.description)}
                                                >
